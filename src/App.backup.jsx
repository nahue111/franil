...
function Hero() {
  ...
  return (
    <section ref={heroRef} className="relative min-h-[100dvh] flex items-center justify-center overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-6xl mx-auto px-6 pt-28 pb-20 text-center">
        ...
        <div ref={ctaRef} className="flex flex-col sm:flex-row items-center justify-center gap-4">
          <a
            href="#servicios"
            className="flex items-center gap-2.5 px-8 py-4 bg-franil-accent text-franil-dark font-bold text-base rounded-2xl hover:bg-amber-400 transition-all duration-200 active:scale-[0.98]"
          >
            Ver servicios
            <ArrowRight size={18} />
          </a>
          <a
            href="#proyectos"
            className="flex items-center gap-2.5 px-8 py-4 glass-card text-franil-text font-semibold text-base rounded-2xl hover:border-franil-accent/30 transition-all duration-200 active:scale-[0.98]"
          >
            Proyectos realizados
            <ChevronRight size={18} className="text-franil-muted" />
          </a>
        </div>
      </div>

      <div className="absolute bottom-0 left-0 right-0 z-10">
        <div className="max-w-6xl mx-auto px-6 pb-12 grid grid-cols-2 md:grid-cols-4 gap-4">
          {STATS.map((s) => (
              ...
              >
                <div
                  ...
                  style={{
                    ...
                    filter: 'grayscale(40%) contrast(115%) brightness(0.3)',
                  }}
                />
                <div className="absolute inset-0 bg-gradient-to-t from-franil-dark via-franil-dark/60 to-transparent" />
...
function VideoReel() {
  ...
  // desktop = landscape video, mobile = portrait video
  const isDesktop = window.matchMedia('(min-width: 768px)').matches
  const frameFolder = isDesktop ? '/frames-desktop' : '/frames'
